...
class CIFParser:
    ...
    def is_in_area_of_interest(self, locations):
        ...
        Also returns which location matched for debug purposes.
        """
        if not self.area_of_interest:
            # If no specific area is set, include all schedules
            return True
            
        # Check each location in this schedule
        matching_locations = []
        ...
        if matching_locations:
            ...
            logger.info(f"Schedule matches area of interest at locations: {', '.join(matching_locations)}")
            return True
